...
```
3. **泛型接口**: 接口通过定义泛型的方式增加灵活性
```typescript
interface CreateId<T> {
  id: (value: T) => T;
  ids: (value: T[]) => T[];
}
let obj: CreateId<number> = {
  id: (value: number) => value, //返回的类型为number
  ids: () => [1, 2, 3] //返回的类型为number[]
}
```
当外面再使用数组时，TS会根据数组的不同类型，自动将类型变量设置为响应的类型。
4. **泛型类**:
```typescript
...
const getNumber = new GetNumber<number>();
getNumber.default = 0;
```
5. **泛型工具类型**:
   `Partial<Type>`: 创建一个类型，将传入的类型的属性变为可选。
   `readonly<Type>`: 将传入的属性变为只读。
   `Pick<Type, Keys>`: 从Type中选择一组属性来构建新类型。
   `Record<Keys, Type>`: 创建一个对象，对象属性为Keys，属性值类型为Type。
```typescript
//关于Partial<Type>
interface Props {
  id: number;
...
type Props2 = readonly<Props2>;
let p2: Props2 = { id: 1, children: [1, 2, 3] }; //此时属性变为只读，无法重新赋值
...
type Props3 = Pick<Props3, 'id' | 'name'>; //第二个变量中传入的属性只能是第一个类型办理中存在的属性

//关于Record<Keys, Type>
type RecordObj = Record< 'id' | 'name', string[]>; //第一个变量表示可以传入哪些属性，第二个表示属性对应的类型
let record: RecordObj = { id: ['1'], name: ['2'] };
```
6. **索引签名类型**
当无法确定对象中有哪些属性时，就可以使用索引签名类型。
```typescript
interface anyObj {
  [key: string]: string; //只要是string类型的键，都可以作为属性名
}
let obj: anyObj = { name: '张三', age: '18' };

interface anyObj2<T> {
  [key: number]: T;
}
let obj2: anyObj2<number> = { 1, 2, 3 };
```
7. **映射类型**:
   基于旧类型创建新类型
```typescript
//根据联合类型创建
type PropKeys = 'x' | 'y';
type Props = { [Key in PropKeys]: number }; //映射不能在接口中使用

//根据对象类型创建
type PersonObj = { name: string; age: number };
type Person = { [Key in keyof PersonObj]: PersonObj[Key] };
```
### 七、类型声明文件
1. **类型声明文件**: 为已经存在的JS库提供类型信息。
2. **文件类型**: 
   * `.ts`: TypeScript类型声明文件
     既包含类型信息，又可执行代码。
     可以被编译成`.js`文件，然后执行
   * `.d.ts`: 只包含TypeScript类型信息声明文件
     不会生成`.js`文件，只负责提供类型信息
   安装lodash：`npm i @type/lodash --save-dev`
3. **创建类型声明文件**:
    创建一个`.d.ts`文件，并添加类型声明信息
    创建需要共享的类型，使用`export`关键字导出
    需要使用共享类型的.ts文件中，用`import`导入
