...
Component({
   observers: {
      '对象.属性1, 对象.属性2': function(onj) { //支持使用通配符，例如 '对象.**'
         this.setData({
            功能逻辑
         })
      }
...
```
### 十四、纯数据字段
纯数据字段，即数据字段的值不会触发视图更新，也不传递给其他组件。用于提升页面更新的性能。
在options中添加`pureDataPattern`属性，值为正则表达式，匹配的字段则为纯数据字段。
```js
Component({
   options: {
      pureDataPattern: /^_/ // 匹配以_开头的字段
   },
   data: {
      _number: 0 // 纯数据字段
      number: 0 // 非纯数据字段
...
1. **created**: 组件实例刚刚被创建时执行。
   此时不能调用setData()方法。
   只能用于给组件的this添加自定义属性。
2. **attached**: 组件实例被添加到页面节点树时执行。
   此时，this.data已经初始化完毕。
   绝大多是初始化的操作都会在该阶段执行。
3. **ready**: 在组件视图布局完成后执行。
4. **moved**: 组件实例被移动到树的另一个位置时执行。
5. **detached**: 组件实例从页面节点树中移除时执行。
   退出页面时，会触发每个自定义组件的detached生命周期。
   适合做清理数据工作。
6. **error**: 当组件方法抛出错误时执行。
**生命周期函数一般定义在lifetimes字段中** 
**组件所在页面的生命周期:**
自定义组件的行为依赖于页面状态的变化，此时需要用到组件所在页面的生命周期。
1. **show**: 组件所在页面被展示时执行。
2. **hide**: 组件所在页面被隐藏时执行。
3. **resize**: 组件所在页面尺寸变化时执行。
**组件所在页面的周期函数需要定义在pageLifetimes字段中**
```js
Component({
   lifetimes: {
      attached: function() { // 组件实例被添加到页面节点树时执行
      }
   },
   pageLifetimes: {
      show: function() { // 组件所在页面被展示时执行
      }
   }
})
```
### 十六、插槽
在wxml结构中提供一个`<slot>`标签，用于承载组件的子节点。
对于不确定的组件，使用`<slot>`标签，将子节点插入到`<slot>`标签中。
```html
...
```
若需要声明多个插槽，则需要先在页面js的options中添加`"multipleSlots": true`，随后即可在页面中添加多个插槽
此时，在页面中定义多个插槽时，需要使用`<slot name="slotName">`标签，其中`name`属性为插槽的名称。 
### 十七、组件数据通信
**父子间组件之间通信**
1. **属性绑定**: 用于父组件向子组件的指定属性设置数据，仅支持JSON兼容的数据。
   子组件在properties中声明对应属性并使用，
   父组件在wxml中通过`<子组件属性名="属性值">`绑定数据。
   ```html
   <!-- 父组件中绑定数据 -->
   ...
   Component({
      properties: {
         number: Number // 父组件中绑定的属性类型
      }
   })
   ```
   在子组件中就可以通过使用`{{ number }}`来获取父组件中绑定的数据。
2. **事件绑定**: 用于子组件向父组件传递数据，可支持任意数据。
   事件绑定步骤如下:
   1.在父组件的js中，定义一个函数，这个函数即将通过自定义事件的形式传递给子组件。
   2.在父组件的wxml中，通过自定义组件的形式，将步骤1中的函数引用传递给子组件。
   3.在子组件js中，通过`this.triggerEvent('自定义事件名称', {/*参数对象*/})`方法，把数据传递给父组件。
   4.在父组件js中，通过`event.detail`获取子组件传递的数据。
3. **获取组件实例**: 父组件通过`this.selectComponent("id或class选择器")`方法获取子组件实例，并调用子组件实例。可以直接访问子组件的任意数据和方法。
```html
<!-- 子组件 -->
...
```javascript
// 父组件
const myComponent = this.selectComponent(".my-component"); //此时可通过myComponent访问子组件的任意数据和方法
```
4. **自定义组件的behaviors**
   用于实现组件之间代码的共享
   behavior包含一组属性、数据、生命周期函数和方法，组件引用他时，它的属性、数据、方法会被合并到组件中。
   每个组件可以引用多个behavior，behavior也可以引用其他behavior。
   调用Behavior(Object)方法即可创建一个共享的behavior实例对象，供所有组件使用:
   ```javascript
   module.exports = Behavior({
       data: {}, // 组件数据，是私有的，在组件外部无法直接访问
       methods: {}, // 组件方法
       properties: {}, // 组件属性
       ... // 组件其他属性
   })
   ```
   需要使用时，使用`require()`方法引入behavior实例对象：
   ```javascript
   ...
   2.`methods`: 组件方法
   3.`properties`: 组件属性
   4.`behaviors`: 引入其他组件的behavior
   5.`created`: 组件创建时执行
   6.`attached`: 组件插入页面时执行
   7.`ready`: 组件渲染完成时执行
   8.`moved`: 组件移动到页面时执行
   9.`detached`: 组件移出页面时执行
### 十八、小程序中的npm
小程序中支持使用npm安装第三方包，但有限制:
1. 不支持依赖于`Node.js`内置库的包
2. 不支持依赖于浏览器内置对象的包
3. 不支持依赖于`C++`的包
### 十九、uni-app
uni-app 是一个使用 Vue 创建的跨平台小程序解决方案，它提供了一套完整的 API，使得开发者可以在同一套代码 basis 上开发 Android、iOS、H5 三端的小程序。
1. **目录结构**:
   `components`: 组件目录
   `pages`: 页面目录
   `static`: 静态资源目录
   `main.js`: Vue初始化入口文件
   `app.vue`: 应用配置
   `manifest.json`: 应用配置/appid等信息
   `pages.json`: 页面配置路径
2. **项目运行到微信小程序开发工具**:
   1.获取微信小程序appid，填入`manifest.json`中
   2.在HBuilder中配置微信小程序开发工具安装路径
   3.在微信小程序开发工具安全设置中开启端口服务
   4.运行项目，选择运行到微信小程序开发工具
