...
2. **React.memo与prop的比较机制**:
   ...
   ```
### 十九、关于useCallback
1. **`useCallback`**: 在组件多次渲染时，缓存函数，避免重复创建函数。
   用法同memo，不过memo用于缓存值，useCallback用于缓存函数。
### 二十、关于forwardRef
1. **`forwardRef`**: 通过ref获取到子组件的input元素, 让其聚焦
   ```jsx
   // 子组件
   ...
   ```
### 二十一、关于useImperativeHandle
1. **`useImperativeHandle`**: 允许在自定义组件中，通过ref获取组件的实例。
   例: 通过ref调用input子组件内部的focus方法实现聚焦
   ```jsx
   // 子组件
   const Child = forwardRef((props, ref) => { 
      const inputRef = useRef(null)
   ...
   ```
### 二十二、关于class API
1. **class API**: 通过创建类来组织组件的代码
2. **使用**:
   (1)通过类属性state定义状态数据
   (2)通过this.setState修改状态数据
   (3)通过render方法写UI模板
   **以自增按钮为例**
   ```jsx
   ...
   ```
3. **类组件的生命周期**:
   (1)`componentDidMount`: 组件挂载后执行, 异步数据获取
   (2)`componentDidUpdate`: 组件更新后执行, 状态数据更新后执行
   (3)`componentWillUnmount`: 组件卸载前执行, 清理定时器、取消网络请求
4. **类组件的组件通信**: 和hooks组件的组件通信一样
### 二十三、关于zustand
1. **zustand**: 状态管理库
2. **使用**:
   ...
   const useStore = create((set) => {
      return {
         ...
         getChannelList: async () => {
            ...
            // 使用set更新状态
            set({channelList: jsonData.data})
         }
      }
   })
   ``` 
4. **切片模式**
   当单个store过多时，可以使用切片模式进行拆分
   ```js
   import {create} from 'zustand';
   // 创建counter切片
   ...
   ```
### 二十四、React+TypeScript
1. **创建环境**: `npm create vite@latest my-project --template react-ts`
2. **useState自动推导**: React会根据useState的默认值，自动推导state的类型，不需要显式标注
3. **useState传递泛型参数**: 
   ```tsx
   // 限制useState的函数参数初始值必须满足类型为User | () => User
   // 限制setUser的参数必须满足类型为User | () => User | undefined
   // user状态数据具备User类型的类型提示
   type User = { name: string; age: number };
   const [user, setUser] = useState<User>();
   ``` 
4. **useState初始值为null**: 
   当不知道初始值是什么，可以将useState的参数设置为null
   ```tsx
   // 限制useState的函数参数初始值必须满足类型为User | null
   // 限制setUser的参数必须满足类型为User | null 
   type User = { name: string; age: number };
   const [user, setUser] = useState<User | null>(null);
   ```
5. **prop和typescript**: 
   为组件prop添加类型约束，本质是给函数的参数做类型注解，可以使用type对象或interface接口做注解。
   **为children添加类型约束**: 需要通过内置的React.ReactNode类型做约束
   children可以是多种类型，如string、ReactElement、ReactFragment、ReactPortal、boolean、null、undefined
   ```tsx
   type Props = { name: string, children: React.ReactNode };
   function App(props: Props) {
      const { name, children } = props;
      {/* button只能传入名称为"name"的prop参数，类型为string，必填 */}
      return <button name={name}>点击{children}</button>;
   }
   ```
   **为事件prop添加类型**:
   在组件内部调用时需要遵守类型约束，参数传递应当满足要求
   绑定prop时如果绑定内联函数直接可以推断参数类型，否则需要单独注解匹配的参数类型
   ```tsx
   type Props = {
      onGetMsg: (msg: string) => void; // 函数参数类型为string
   }
   function Son (props: Props) {
      ...
      return <button onClick={handleClick}>发送信息</button>
   }
   // 父组件
   function App () {
   ...
   ```
6. **useRef和TypeScript**:
   **获取dom**: 可以把要获取的dom元素当成泛型参数传递给useRef, 推导出`.current`的类型
   ```tsx
   function App() {
   ...
   ``` 
   **引用稳定的存储器**: -把useRef当成引用稳定的存储器使用的场景可以通过传递联合类型来做，比如定时器:
   ```tsx
   function App() {
