...
2. **使用Redux**: 
   ...
   (2)定义一个`reducer`函数(根据当前想做的修改返回一个全新的状态)
   (3)使用`createStore`方法传入`reducer`函数，创建一个`store`对象
   (4)使用`store.subscribe`方法订阅`store`对象，监听状态的变化，数据变化即可得到通知
   (5)使用`store.dispatch`方法提交一个`action`对象，触发数据变化(告诉`reducer`应当如何改数据)
   (6)使用`store.getState`方法获取当前最新状态更新到视图中
