...
* 拷贝的是地址，而不复制属性中的对象。
```javascript
const obj1 = {name: 'obj1', age: 18};
const o = {...obj1};
console.log(o); //{name: 'obj1', age: 18}
o.age = 20;
console.log(o); //{name: 'obj1', age: 18}
console.log(obj1); //{name: 'obj1', age: 20} 浅拷贝可防止obj1的属性被修改
//还可以使用下面的方法实现浅拷贝
const o2 = {};
...
function deepClone(newObj, oldObj) {
  for (let key in oldObj) {
    if (oldObj[key] instanceof Array){
      newObj[key] = [];
      deepClone(newObj[key], oldObj[key]); // 递归的方式完成深拷贝数组 
    } else if (oldObj[key] instanceof Object) { 
      newObj[key] = {};
      deepClone(newObj[key], oldObj[key]); // 递归的方式完成深拷贝对象
    } else{
...
### 十九、异常处理
1. **throw new Error()** 中断运行
2. **try...catch...finally...** 捕获异常，不中断运行，需要加`return`手动中断运行 
3. **debugger** 暂停运行，调试代码
### 二十、改变this指向
1. **call**、**apply**、**bind** 改变this指向，
   `fun.call(thisArg, arg1, arg2, ...)`第一个参数在函数运行时指定this，arg1, arg2...为函数需要的其他参数
   `fun.apply(thisArg, [arg1, arg2, ...])`同call，但是参数为数组，可用于求最值 `Math.max.apply(Math, [1, 2, 3])`
   `fun.bind(thisArg, arg1, arg2, ...)`参数同call，但不会调用函数
### 二十一、防抖
1. **函数防抖：** 单位时间内，频繁触发事件，只执行最后一次
   **lodash实现防抖：** 
   `_.debounce(func, wait, [options])` 第一个函数是指要处理的函数，第二个参数是等待时间(单位是ms)。
   **原理：**
```javascript
...
```
### 二十二、节流
1. **节流：** 单位事件内，频繁触发事件，只执行一次。
2. **lodash实现节流：** `_.throttle(func, wait, [options])` 第1个参数是指要处理的函数，第二个参数是等待时间(单位是ms)。
 **原理：**
```javascript
function throttle(func, wait) {
  ...
  return function () {
    if (!timeout) {
      timeout = setTimeout(() => {
        func();
        timeout = null; //不能使用clearTimeout，在开启的定时器中不能清除定时器
      }, wait)
    }
  }
}
